...
def say_hello_at_5_journalists(client)
  ...
  five = journalists.sample(5)
  five.each do
    client.update("#{five} Vous avez été choisis parmi mon algorithme Ruby. Voilà c'est tout, passez une agréable journée. #bonjour_monde @the_hacking_pro")
    puts client
  end
end
  
puts say_hello_at_5_journalists(client)
...
def say_hello_at_5_journalists(client)
  client.search('#bug').take(3).each do |tweet|
    client.update("@#{tweet.user.screen_name} Hello there")
  end
end
